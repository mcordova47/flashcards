import { deckFingerprint, formatVersion, slugAt, storedAt } from "./harness.mjs"
...
export default async ({ check, open }) => {
  ...
  const edge = await open({ seed: seed(
    { slug: slugAt(191), box: 3, seen: 4, missed: 1, lapses: 0, direction: "recognition", due: overdue() }) })
  ...
  const graduated = storedAt((await edge.stored()).cards, 191)
  ...
  const sibling = await open({ seed: seed(
    { slug: slugAt(254), box: 3, seen: 4, missed: 1, lapses: 0, direction: "recognition", due: overdue() }) })
  ...
  const barred = storedAt((await sibling.stored()).cards, 254)
  ...
  const many = await open({ seed: seed(
    { slug: slugAt(191), box: 1, seen: 5, missed: 1, lapses: 0, direction: "production", due: overdue() }) })
  ...
  const one = await open({ seed: seed(
    { slug: slugAt(8), box: 1, seen: 5, missed: 0, lapses: 0, direction: "production", due: overdue() }) })
  ...
  const climbed = storedAt((await one.stored()).cards, 8)
  ...
  // Progress from before the rule existed: rank 254 already graduated.
  {
    const stale = await open({ seed: { version: formatVersion(), deck: FP, cards: [
      { slug: slugAt(191), box: 2, seen: 9, missed: 2, lapses: 1, direction: "production", due: overdue() },
      { slug: slugAt(254), box: 3, seen: 7, missed: 3, lapses: 2, direction: "production", due: overdue() + 1000 },
    ] } })
    ...
    const moved = storedAt(cards, 254)
    ...
    check("and the fix is written straight away, not on the next answer",
      storedAt((await stale.stored()).cards, 254).direction, "recognition")
  ...
  const slipped = await open({ seed: seed(
    { slug: slugAt(8), box: 3, seen: 9, missed: 1, lapses: 1, direction: "production", due: overdue() }) })
  await slipped.waitForSelector(".prompt")
  await slipped.tap(".card")
  await slipped.tap(".again")
  const missed = storedAt((await slipped.stored()).cards, 8)
